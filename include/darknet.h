...
/*-------------------- utls.c -------------------*/
/* func: read the whole file into memory.
 * (the caller is responsible to free the newly allocated memory)
 */
unsigned char *read_file(const char *filename);

/* find a argument which is of int type
 */
int find_int_arg(int argc, char **argv, char *arg, int def);

/* func: find a argument which is of float type.
 */
float find_float_arg(int argc, char **argv, char *arg, float def);

/* func: find a argument which is equivalent to 'arg'.
 */
int find_arg(int argc, char* argv[], char *arg);

/* func: find a argument which is of string type
 */
char *find_char_arg(int argc, char **argv, char *arg, char *def);

/* func: return the file name without ext-suffix and parent-path.
...
/* func: read each line from 'filename', and convert it into integer.
 */
int *read_map(const char *filename);

void error(const char *s);

/* func: return the index of the maximum element.
 */
int max_index(float *a, int n);

/* func: integer-version of 'max_index'
 */
int max_int_index(int *a, int n);

/* func: return the index of the random selected element in the array 'a'.
 */
int sample_array(float *a, int n);

/* func: return a random array with all the elements are in the range of [min, max].
 * (the caller is responsible to free the newly malloced memory of this array)
 */
int *random_index_order(int min, int max);

/* func: calculate MSE value of vector 'a'.
 */
float mse_array(float *a, int n);

/* func: calculate variance value of vector 'a'.
 */
float variance_array(float *a, int n);

/* func: calculate magnitude(length) of the vector 'a'.
 */
float mag_array(float *a, int n);

/* func: a = a * s (a is a vector)
 */
void scale_array(float *a, int n, float s);

/* func: calculate the mean value of vector 'a'.
 */
float mean_array(float *a, int n);

/* func: calculate the sum value of vector 'a'.
 */
float sum_array(float *a, int n);

/* func: normalize a vector.(after nomalization: mean=0, variance=1.0)
 */
...
/* func: return a random number, whose type is size_t.
 */
size_t rand_size_t();

/* func: return a random number, which confirms to normal-distribution.
 */
float rand_normal();

/* func: return a random number, which confirms to uniform-distribution.
 */
float rand_uniform(float min, float max);

/*-------- see the details in utils.h  start ----------*/
void find_replace(const char *str, const char *orig, const char *rep, char *output);
void free_ptrs(void **ptrs, int n);
char *fgetl(FILE *fp);
void strip(char *s);
float sec(clock_t clocks);
/*-------- see the details in utils.h  end ----------*/
